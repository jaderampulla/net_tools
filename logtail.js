...
function stopTail() {
	clearTimeout(t);
	var pause = "Live log paused. Click \"Start Log\" to restart.\n";
	logDiv = document.getElementById("log");
	var newNode=document.createTextNode(pause);
	logDiv.insertBefore(newNode,logDiv.childNodes[0]);
}
	...
	if (request.readyState == 4) {
		if (request.status == 200) {
			...
			for (i=0; i < currentLogValue.length - 1; i++) {
				logLine += currentLogValue[i];// + "<br/>\n"; //Removed because we're outputting a table with formatting and don't need lines
			}
